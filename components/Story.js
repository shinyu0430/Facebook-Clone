...
const Story = () => {
  ...
  const generateFakeData = () => {
    ...
    for (let i = 0; i < 10; i++) {
      const title = dogNames[i % dogNames.length];
      const img = `https://source.unsplash.com/featured/?dog&random=${i}`;
      const postImg = `https://source.unsplash.com/featured/?dog+toy&random=${i}`;
      fakeData.push({ title, img, postImg });
    }
    return fakeData;
  };
  

  // const singleStoryData = generateFakeData();
